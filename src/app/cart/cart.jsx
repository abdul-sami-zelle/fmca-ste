...
import MetaCartSnackbar from '@/UI/Components/ProductNotAvailableSnackbar/productNotAvailableSnackBar';





const Cart = ({ isMetaShop, metaProducts, unavailableProducts }) => {
    ...
    console.log(
    "CART RECEIVED META PRODUCTS:",
    metaProducts
);

console.log(
    "CART RECEIVED META PRODUCTS IS ARRAY:",
    Array.isArray(metaProducts)
);
    const {
        shippingMethods,
        info,
...
useEffect(() => {
    if (metaProducts.length > 0) {
        addMetaProductsToCart(metaProducts);
    }
}, [metaProducts]);
    useEffect(() => {
        if (shippingMethods) {
