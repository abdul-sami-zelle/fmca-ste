...
// const ProductDisplay = ({ params, otherData }) => {

//   const { slug } = use(params);
//   const { singleProductData, selectedVariationData, setSelectedVariationData,
//     setSelectedVariationUid, setSingleProductData, findObjectByUID } = useProductPage();

//   const [product, setProduct] = useState(otherData || null);


//   // ✅ Derive initial variation synchronously from otherData
//   const getInitialVariation = () => {
//     if (!otherData || otherData.type !== 'variable') return null;
//     const defaultUid = otherData.default_variation;
//     return otherData.variations?.find(v => v.uid === defaultUid) || 
//            otherData.variations?.[0] || null;
//   };

//   // ✅ Use initializer function so useState only runs once
//   const [localVariationData, setLocalVariationData] = useState(getInitialVariation);
  
//   // Use localVariationData as fallback while context hydrates
//   const activeVariation = selectedVariationData || localVariationData;
...
const ProductDisplay = ({ params, productAPIData }) => {
  ...
  const [product, setProduct] = useState(productAPIData || null);
  const [showDesignRoomModal, setShowDesignRoomModal] = useState(false);
  const [productDetails, setProductDetails] = useState({});
  const [isSticky, setIsSticky] = useState(false);
  const [variationData, setVariationData] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const tabBarItems = [
    ...(product?.type === 'variable'
      ? selectedVariationData?.dyrc?.active === 1 ? ['DesignYourRoom'] : []
      : product?.dyrc?.active === 1 ? ['DesignYourRoom'] : []),
    'Description',
    'Details'
  ];
  ...
  const showDRM = () => setShowDesignRoomModal(true);
  const closeDRM = () => setShowDesignRoomModal(false);
  ...
  useEffect(() => {
    setProductDetails({
      ...
      brand: product?.brand !== '' ? product?.brand : 'Furniture Mecca',
      category: product?.categories?.find(item => item.is_main === 1)?.name,
      stock: product?.manage_stock?.stock_status?.toLowerCase() === 'instock'
        ? 'In Stock'
        : 'Back Order',
      mpn: product?.mpn,
      gtin: product?.gtin,
  ...
  // Sync context when slug changes (e.g. navigating between product pages)
  useEffect(() => {
    if (!productAPIData) return;
    setSingleProductData(productAPIData);
    setSelectedVariationUid(productAPIData?.default_variation);
    const defaultVariation = findObjectByUID(productAPIData?.default_variation, productAPIData?.variations);
  ...
  const handleOpenModal = (place, type) => {
    setZoomIn(false);
    setClickedType(type === 'dimenssion-show' ? type : 'dimenssion-hide');
    setGalleryModalWidth(place === 'image-clicked');
    setDimensionModal(true);
  };
      ...
      if (thumbnailContainerRef.current) {
        if (typeof window !== 'undefined') {
          if (window.innerWidth < 480) {
            ...
            thumbnailContainerRef.current.scrollBy({ top: -80, behavior: 'smooth' });
          }
        }
      }
      return newIndex;
  ...
  const handleNextImage = () => {
    setActiveIndex((prevIndex) => {
      const length = product.type === 'variable'
        ? selectedVariationData?.images?.length + 1
        : product?.images?.length;
      if (prevIndex === length) return prevIndex;
      const newIndex = prevIndex + 1;
      setThumbActiveIndex(newIndex);
      setCurrentIndex(newIndex);
      if (thumbnailContainerRef.current) {
  ...
  useEffect(() => {
    document.body.style.overflow = dimensionModal ? 'hidden' : 'auto';
  }, [dimensionModal]);

  useEffect(() => {
    document.body.style.overflow = showDesignRoomModal ? 'hidden' : 'auto';
  }, [showDesignRoomModal]);

  useEffect(() => {
    if (dimensionModal) handleThumbnailClick(activeIndex);
  }, [dimensionModal]);

  const stockCheck = product?.type === 'variable'
    ? (
      (selectedVariationData?.manage_stock?.stock_status === 'inStock' && selectedVariationData?.manage_stock?.quantity === 0)
      || selectedVariationData?.manage_stock?.stock_status === 'outStock'
      || selectedVariationData?.manage_stock?.stock_status === 'outOfStock'
    )
    : (
      (product?.manage_stock?.stock_status === 'inStock' && product?.manage_stock?.quantity === 0)
      || product?.manage_stock?.stock_status === 'outStock'
      || product?.manage_stock?.stock_status === 'outOfStock'
    );

  const isVariableOrNot = product?.type === 'variable';

  const isDesignRoomActive = product?.type === 'variable'
    ? selectedVariationData?.dyrc?.active === 1
    : product?.dyrc?.active === 1;
  ...
  return (
    <div ref={produtPageRef}>
      ...
      {showDesignRoomModal && (
        <div className='design_room_main_modal'>
          <DesignRoomMain
            closeFn={closeDRM}
            product={product}
            data={
